...
import { orderStatus, paymentStatus } from 'src/schema/orders';
...
export class OrdersService {
  constructor(
    ...
    @Inject(OrdersRepository) private readonly ordersDB: OrdersRepository,
  ...
  async create(createOrderDto: Record<string, any>) {
    try {
      const orderExists = await this.ordersDB.findOne({
        checkoutSessionId: createOrderDto.checkoutSessionId,
      });
      if (orderExists) return orderExists;

      const result = await this.ordersDB.create(createOrderDto);
  ...
  async findAll(status: string, user: Record<string, any>) {
    try {
      ...
      const orders = await this.ordersDB.find(query);
  ...
  async findOne(id: string) {
    try {
      const result = await this.ordersDB.findOne({ _id: id });
  ...
  async fullfillOrder(
    ...
    try {
      return await this.ordersDB.findOneAndUpdate(
        { checkoutSessionId },
        updateOrderDto,
        { new: true },
      );
